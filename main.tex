...
%\usepackage{subfiles}
%\def\biblio{\bibliographystyle{RS}\bibliography{references}}
...
\end{equation}
The goal is to find an $\alpha$ which maximises the expected net revenues over a
given time horizon $T$,
\begin{equation}
  \max_{\alpha}\mathbb E_W\left[ \sum_{t=0}^TU_t(S_t,\alpha_t,W_{t+1})
    - \overline{U}(S_T)\right],
\end{equation}
where $U_t$ denotes the revenue at time $t$, and $\overline{U}$ the
...
Consider a system over discrete, equispaced, non-dimensional time points
$t=0,1,\dots,T$, with state
$S_t$ and pricing process $\alpha$ such that the $\alpha_t$ that take values in a closed
interval $A=[a_{\mathrm{min}},a_{\mathrm{max}}]$ of prices.\footnote{In
...
stock $C\geq 0$.\footnote{In some situations, unsold items at time $T$
  ...
  $C<0$. We generally assume $C\geq 0$ in this paper, however.}
...
\begin{align}\label{eq:value_function_def_nondim}
  ...
  \hat J(\hat t,\hat s,\hat \alpha)&=
                                     ...
                                     {S_\tau^\alpha},\hat{\alpha}_\tau,\hat
                                     W_{\tau+1})
...
linear interpolation for $I[v^K](s)$ in step~(iv)(c).
...
range of parameters in Subsection~\ref{sec:suboptimal_approximations}\ref{sec:parameter_comparison}.
...
From the optimality of the Bellman policy function, we should have
...
The parameters chosen in our numerical experiments
are $C=1$, $\gamma=0.05$, $q_1=e^2/3\approx2.5$, and $q_2=3$. This point lies
