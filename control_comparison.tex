...
estimate of the system \citep[Ch.~6]{bertsekas2005dynamic}. For the
...
For each decision point $t=0,\dots,T-1$, the CEC algorithm for the pricing
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Comparison of the CEC policy function $a^C$ and
    the optimal Bellman policy function $a^B$, see also
    \Cref{fig:markdown_bellman}.
    The CEC function sets
    higher prices than the Bellman function for large $s$.
    Observe that when $t<T-1$, there is a small region where
    $a^B(t,s)\geq a^C(t,s)$, before they
    both reach the boundary price $a_{max}=1$. This is due to the
    diffusion of the value function by the
...
However, from the histogram we see that
...
\begin{figure}[htbp]
  ...
  \caption{This shows the distribution from 1000 samples of
    the profit of following the Bellman and CEC policies.
